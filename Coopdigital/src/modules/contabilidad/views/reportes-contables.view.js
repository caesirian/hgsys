...
function agruparPorCategoria(rows, tipo) {
  const filtrados = rows.filter(r => r.tipo === tipo);
  const totales = {};
  filtrados.forEach(r => { totales[r.categoria] = (totales[r.categoria] ?? 0) + Number(r.monto); });
...
export async function bindReportesContables() {
  ...
  function renderReporte() {
    ...
    const totalIngresos = ingresos.reduce((a, [, m]) => a + m, 0);
    const totalEgresos = egresos.reduce((a, [, m]) => a + m, 0);
    const resultado = totalIngresos - totalEgresos;

    content.innerHTML = `
      <div class="grid-2">
        <div class="card">${renderTabla('Recursos (ingresos)', ingresos, totalIngresos)}</div>
        <div class="card">${renderTabla('Gastos (egresos)', egresos, totalEgresos)}</div>
      </div>
      <div class="card resultado-card">
        <span class="muted">Resultado del período</span>
        <div class="resultado-valor ${resultado >= 0 ? 'positivo' : 'negativo'}">${money(resultado)}</div>
      </div>`;
  }
  ...
  document.querySelector('#exportCsv').onclick = () => {
    ...
    downloadCsv('estado-recursos-gastos.csv', filtrados.map(m => ({
      fecha: m.fecha, tipo: m.tipo, categoria: m.categoria, monto: m.monto, descripcion: m.descripcion
    })));
  };
