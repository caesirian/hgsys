...
const columns = [
  ...
  { key: 'categoria',  label: 'Categoría', render: r => `<span class="badge">${escapeHtml(r.categoria)}</span>` },
  ...
  { key: 'url', label: 'Link', render: r => r.url && isSafeUrl(r.url)
      ? `<a href="${escapeHtml(r.url)}" target="_blank" rel="noreferrer">Abrir</a>`
      : '—' }
];
